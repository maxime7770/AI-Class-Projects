...
class RegressionModel(object):
    ...
    def run(self, x):
        ...
        y3 = nn.AddBias(nn.Linear(y2, self.w3), self.b3)
        return y3
    ...
    def train(self, dataset):
        ...
        while True:
            for x, y in dataset.iterate_once(self.batch_size):
                loss = self.get_loss(x, y)
                grad_w1, grad_w2, grad_b1, grad_b2, grad_w3, grad_b3 = nn.gradients(loss, [self.w1, self.w2, self.b1, self.b2, self.w3, self.b3])
                self.w1.update(grad_w1, -self.learning_rate)
                self.w2.update(grad_w2, -self.learning_rate)
                self.b1.update(grad_b1, -self.learning_rate)
                self.b2.update(grad_b2, -self.learning_rate)
            ...
            if validation_acc > 0.97:
                self.learningrate = 0.01
            if validation_acc > 0.975:
                return
...
class LanguageIDModel(object):
    ...
    def __init__(self):
        ...
        self.w3 = nn.Parameter(256, 256)
        self.w4 = nn.Parameter(256, 5)
    ...
    def run(self, xs):
        ...
        for i in range(len(xs)):
            if i == 0:
                h = nn.ReLU(nn.AddBias(nn.Linear(xs[i], self.w1), self.b1))
            else:
                h = nn.ReLU(nn.Add(nn.AddBias(nn.Linear(xs[i], self.w2), self.b2), nn.AddBias(nn.Linear(h, self.w3), self.b3)))
        return nn.AddBias(nn.Linear(h, self.w4), self.b4) 
    ...
    def train(self, dataset):
        ...
        while True:
            for x, y in dataset.iterate_once(self.batch_size):
                ...
                self.w2.update(grad_w2, -self.learning_rate)
                self.w3.update(grad_w3, -self.learning_rate)
                self.w4.update(grad_w4, -self.learning_rate)
                self.b1.update(grad_b1, -self.learning_rate)
                self.b2.update(grad_b2, -self.learning_rate)
                self.b3.update(grad_b3, -self.learning_rate)
            ...
            validation_acc = dataset.get_validation_accuracy()
            print("Validation accuracy: ", validation_acc)
            if validation_acc > 0.75:
                self.learning_rate = 0.05
            if validation_acc > 0.80:
                self.learning_rate = 0.03
            if validation_acc > 0.85:
                self.learning_rate = 0.01
            if validation_acc > 0.89:
                return
