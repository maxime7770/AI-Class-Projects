...
def sentence3() -> Expr:
    ...
    return conjoin([(PacmanAlive_1 % disjoin([conjoin([PacmanAlive_0, ~(PacmanKilled_0)]), \
...
def findModelCheck() -> Dict[Any, bool]:
    ...
    "*** BEGIN YOUR CODE HERE ***"
    # do not use findModel or Expr in this method
...
def atMostOne(literals: List[Expr]) -> Expr:
    ...
    """
    "*** BEGIN YOUR CODE HERE ***"
    return conjoin([disjoin([~(i), ~(j)]) for i, j in itertools.combinations(literals, 2)])
    "*** END YOUR CODE HERE ***"
...
def pacphysicsAxioms(t: int, all_coords: List[Tuple], non_outer_wall_coords: List[Tuple], walls_grid: List[List] = None, sensorModel: Callable = None, successorAxioms: Callable = None) -> Expr:
    ...
    for x, y in all_coords:
        ...
        pacphysics_sentences.append(wall >> (~pos))
...
def checkLocationSatisfiability(x1_y1: Tuple[int, int], x0_y0: Tuple[int, int], action0, action1, problem):
    ...
    new_loc = PropSymbolExpr(pacman_str, x1, y1, time=1)

    model1 = findModel(conjoin(KB) & new_loc)
    model2 = findModel(conjoin(KB) & (~new_loc))
...
def positionLogicPlan(problem) -> List:
    ...
    for t in range(50):
        ...
        find = findModel(conjoin(KB) & PropSymbolExpr(pacman_str, xg, yg, time=t))
        if find:
            return extractActionSequence(find, actions)
...
def foodLogicPlan(problem) -> List:
    ...
    for t in range(50):
        ...
        goal = ~disjoin(food_t)
        find = findModel(conjoin(KB + [goal]))
        if find:
            return extractActionSequence(find, actions)
...
def slam(problem, agent) -> Generator:
    ...
    for t in range(agent.num_timesteps):
        ...
        possible_locations = []
        for x, y in non_outer_wall_coords:
            if not entails(conjoin(KB), ~PropSymbolExpr(pacman_str, x, y, time=t)):
